package com.ly.travel.institute.proxy.server.impl;
...
import static com.ly.travel.institute.proxy.server.impl.ConnectionState.AWAITING_CHUNK;
import static com.ly.travel.institute.proxy.server.impl.ConnectionState.AWAITING_INITIAL;
import static com.ly.travel.institute.proxy.server.impl.ConnectionState.AWAITING_PROXY_AUTHENTICATION;
import static com.ly.travel.institute.proxy.server.impl.ConnectionState.DISCONNECT_REQUESTED;
import static com.ly.travel.institute.proxy.server.impl.ConnectionState.NEGOTIATING_CONNECT;
...
public class ClientToProxyConnection extends ProxyConnection<HttpRequest> {
  ...
  ClientToProxyConnection(final DefaultHttpProxyServer proxyServer, SslEngineSource sslEngineSource, boolean authenticateClients,
    ...
    if (sslEngineSource != null) {
      ...
      encrypt(pipeline, sslEngineSource.newSslEngine(), authenticateClients).addListener(future -> {
        if (future.isSuccess()) {
  ...
  protected ConnectionState readHTTPInitial(HttpRequest httpRequest) {
    ...
    if (httpRequest.decoderResult().isFailure()) {
      ...
      FullHttpResponse response =
          ProxyUtils.createFullHttpResponse(HttpVersion.HTTP_1_1, HttpResponseStatus.BAD_REQUEST, "Unable to parse HTTP request");
  ...
  private ConnectionState doReadHTTPInitial(HttpRequest httpRequest) {
    ...
    currentServerConnection =
        isMitming() || isTunneling() ? this.currentServerConnection : this.serverConnectionsByHostAndPort.get(serverHostAndPort);
    ...
    if (newConnectionRequired) {
      try {
        currentServerConnection =
            ProxyToServerConnection.create(proxyServer, this, serverHostAndPort, currentFilters, httpRequest, globalTrafficShapingHandler);
   ...
   * request is being MITM'd, this method always returns false. The format of requests to a proxy server are defined in
   * RFC 7230, section 5.3.2 (all other requests are considered requests to an origin server):
   * 
   * <pre>
       ...
       GET <a href=
  "https://www.example.org/pub/WWW/TheProject.html">https://www.example.org/pub/WWW/TheProject.html</a> HTTP/1.1
  ...
  private void switchToWebSocketProtocol(final ProxyToServerConnection serverConnection) {
    final List<String> orderedHandlersToRemove = Arrays.asList(HTTP_REQUEST_READ_MONITOR_NAME, HTTP_RESPONSE_WRITTEN_MONITOR_NAME,
        HTTP_PROXY_DECODER_NAME, HTTP_ENCODER_NAME, HTTP_DECODER_NAME);
    if (this.channel.pipeline().get(MAIN_HANDLER_NAME) != null) {
      this.channel.pipeline().replace(MAIN_HANDLER_NAME, "pipe-to-server", new ProxyConnectionPipeHandler(serverConnection));
    }
  ...
  /**
   * If the {@link ProxyToServerConnection} completes its connection lifecycle successfully, this method is called to let
   * us know about it.
   */
   ...
   */
  protected boolean serverConnectionFailed(ProxyToServerConnection serverConnection, ConnectionState lastStateBeforeFailure,
      Throwable cause) {
    resumeReadingIfNecessary();
   ...
   *
   * A {@link ChannelPipeline} invokes the read (Inbound) handlers in ascending ordering of the list and then the write
   * (Outbound) handlers in descending ordering.
   *
  ...
  private void initChannelPipeline(ChannelPipeline pipeline) {
    ...
    // respectively.
    pipeline.addLast(HTTP_DECODER_NAME,
        new HttpRequestDecoder(proxyServer.getMaxInitialLineLength(), proxyServer.getMaxHeaderSize(), proxyServer.getMaxChunkSize()));
   ...
   */
  private void closeConnectionsAfterWriteIfNecessary(ProxyToServerConnection serverConnection, HttpRequest currentHttpRequest,
      HttpResponse currentHttpResponse, HttpObject httpObject) {
  ...
  /**
   * Determines if the remote connection should be closed based on the request and response pair. If the request is HTTP
   * 1.0 with no keep-alive header, for example, the connection should be closed.
   *
   ...
   *
   * <p>
   * If authentication is still required, either because no credentials were provided or the credentials were wrong, this
   * writes a 407 response to the client.
   * </p>
   */
  private boolean authenticationRequired(HttpRequest request) {
    ...
    List<String> values = request.headers().getAll(HttpHeaderNames.PROXY_AUTHORIZATION);
  ...
  private void writeAuthenticationRequired(String realm) {
    ...
    FullHttpResponse response =
        ProxyUtils.createFullHttpResponse(HttpVersion.HTTP_1_1, HttpResponseStatus.PROXY_AUTHENTICATION_REQUIRED, body);
    response.headers().set(HttpHeaderNames.DATE, new Date());
    response.headers().set(HttpHeaderNames.PROXY_AUTHENTICATE, "Basic realm=\"" + (realm == null ? "Restricted Files" : realm) + "\"");
    write(response);
  }
  ...
  private void modifyRequestHeadersToReflectProxying(HttpRequest httpRequest) {
    if (isNextHopOriginServer()) {
      /*
       * We are making the request to the origin server, so must modify the 'absolute-URI' into the 'origin-form' as per RFC
       * 7230 section 5.3.1.
       *
       * This must happen even for 'transparent' mode, otherwise the origin server could infer that the request came via a
       * proxy server.
       */
  ...
  private boolean isNextHopOriginServer() {
    ...
    /*
     * Upstream SOCKS proxies are a special case because they do not parse or modify the HTTP request in any way. If the
     * upstream chained proxy is a SOCKS proxy, we should treat it as if we are connecting directly to the origin server.
     */
    switch (currentServerConnection.getChainedProxyType()) {
      ...
      case SOCKS5:
        return true;
      default:
        LOG.warn("Assuming upstream chained proxy of unknown type " + currentServerConnection.getChainedProxyType()
  ...
  /**
   * Switch the de-facto standard "Proxy-Connection" header to "Connection" when we pass it along to the remote host. This
   * is largely undocumented but seems to be what most browsers and servers expect.
   *
  ...
  /**
   * Tells the client that something went wrong trying to proxy its request. If the Bad Gateway is a response to an HTTP
   * HEAD request, the response will contain no body, but the Content-Length header will be set to the value it would have
   * been if this 502 Bad Gateway were in response to a GET.
   *
   ...
   * request, the response will contain no body, but the Content-Length header will be set to the value it would have been
   * if this Bad Request were in response to a GET.
   *
   * @return true if the connection will be kept open, or false if it will be disconnected
   */
   ...
   * Content-Length header will be set to the value it would have been if this 504 Gateway Timeout were in response to a
   * GET.
   *
  ...
  private boolean respondWithShortCircuitResponse(HttpResponse httpResponse) {
    ...
    // if the response is not a Bad Gateway or Gateway Timeout, modify the headers "as if" the short-circuit response were
    // proxied
  ...
  /**
   * Write an empty buffer at the end of a chunked transfer. We need to do this to handle the way Netty creates HttpChunks
   * from responses that aren't in fact chunked from the remote server using Transfer-Encoding: chunked. Netty turns these
   * into pseudo-chunked responses in cases where the response would otherwise fill up too much memory or where the length
   * of the response body is unknown. This is handy because it means we can start streaming response bodies back to the
   * client without reading the entire response. The problem is that in these pseudo-cases the last chunk is encoded to
   * null, and this thwarts normal ChannelFutures from propagating operationComplete events on writes to appropriate
   * channel listeners. We work around this by writing an empty buffer in those cases and using the empty buffer's future
   * instead to handle any operations we need to when responses are fully written back to clients.
   */
   ...
   *
   * We track statistics on bytes, requests and responses by adding handlers at the appropriate parts of the pipeline (see
   * initChannelPipeline()).
   **************************************************************************/
