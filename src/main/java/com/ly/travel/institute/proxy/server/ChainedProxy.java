...
public interface ChainedProxy extends SslEngineSource {
  /**
   * Return the {@link InetSocketAddress} for connecting to the chained proxy. Returning null indicates that we won't
   * chain.
   * 
  ...
  /**
   * Tell LittleProxy the type of chained proxy that it will be connecting to. This setting determines what type of
   * requests LittleProxy will use to communicate with the chained proxy.
   * 
   ...
   * If true, LittleProxy will call {@link SslEngineSource#newSslEngine()} to obtain an SSLContext used by the downstream
   * proxy.
   * 
