...
public interface MitmManager {
  /**
   * Creates an {@link SSLEngine} for encrypting the server connection. The SSLEngine created by this method may use the
   * given peer information to send SNI information when connecting to the upstream host.
   *
   ...
   * 
   * <p>
   * The serverSslSession is provided in case this method needs to inspect the server's certificates or something else
   * about the encryption on the way to the server.
   * </p>
   * 
   * <p>
   * This is the place where one would implement impersonation of the server by issuing replacement certificates signed by
   * the proxy's own certificate.
   * </p>
   *
