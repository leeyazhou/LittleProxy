...
import com.ly.travel.institute.proxy.server.extras.ProxyProtocolMessage;
...
class ConnectionFlow {
  ...
  private void doProcessCurrentStep(final ProxyConnectionLogger LOG) {
    currentStep.execute().addListener(future -> {
  ...
  private void relayProxyInformation() {
    if (clientConnection.isSendProxyProtocol()) {
      ProxyProtocolMessage proxyProtocolMessage =
          getHAProxyMessage(clientConnection.getClientAddress(), serverConnection.getRemoteAddress());
  ...
  private ProxyProtocolMessage getHAProxyMessage(InetSocketAddress clientAddress, InetSocketAddress remoteAddress) {
    ...
    return new ProxyProtocolMessage(HAProxyProtocolVersion.V1, HAProxyCommand.PROXY, HAProxyProxiedProtocol.TCP4,
        clientAddress.getAddress().getHostAddress(), remoteAddress.getAddress().getHostAddress(), clientAddress.getPort(),
