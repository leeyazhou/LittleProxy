...
public class ProxyUtils {
   ...
   * (https://www.w3.org/Protocols/rfc2616/rfc2616-sec13.html#sec13.5.1). Transfer-Encoding is NOT included in this list,
   * since LittleProxy does not typically modify the transfer encoding. See also
   * {@link #shouldRemoveHopByHopHeader(String)}.
   *
   * Header names are stored as lowercase to make case-insensitive comparisons easier.
   */
  @SuppressWarnings("deprecation") // Don't remove header names from this set until they're removed from Netty, just in case someone's still
                                   // using them.
  private static final Set<String> SHOULD_NOT_PROXY_HOP_BY_HOP_HEADERS = ImmutableSet.of(HttpHeaderNames.CONNECTION.toString(),
      HttpHeaderNames.KEEP_ALIVE.toString(), HttpHeaderNames.PROXY_AUTHENTICATE.toString(), HttpHeaderNames.PROXY_AUTHORIZATION.toString(),
      HttpHeaderNames.TE.toString(), HttpHeaderNames.TRAILER.toString(),
      /*
  ...
  /**
   * Adds the Via header to specify that the message has passed through the proxy. The specified alias will be appended to
   * the Via header line. The alias may be the hostname of the machine proxying the request, or a pseudonym. From RFC
   * 7230, section 5.7.1:
   * 
   * <pre>
  ...
  public static void addVia(HttpMessage httpMessage, String alias) {
    String newViaHeader =
        String.valueOf(httpMessage.protocolVersion().majorVersion()) + '.' + httpMessage.protocolVersion().minorVersion() + ' ' + alias;
  ...
  /**
   * Returns <code>true</code> if the specified string is either "true" or "on" ignoring case.
   * 
   * @param val The string in question.
   * @return <code>true</code> if the specified string is either "true" or "on" ignoring case, otherwise
   *         <code>false</code>.
   */
  ...
  /**
   * Returns <code>true</code> if the specified string is either "false" or "off" ignoring case.
   * 
   * @param val The string in question.
   * @return <code>true</code> if the specified string is either "false" or "off" ignoring case, otherwise
   *         <code>false</code>.
   */
  ...
  /**
   * Returns true if the HTTP message cannot contain an entity body, according to the HTTP spec. This code is taken
   * directly from {@link io.netty.handler.codec.http.HttpObjectDecoder#isContentAlwaysEmpty(HttpMessage)}.
   *
   ...
   * Returns false if the server is expected to indicate the end of the HTTP entity by closing the connection.
   * <p>
   ...
   * @return true if the message will indicate its own message length, or false if the server is expected to indicate the
   *         message length by closing the connection
   */
  public static boolean isResponseSelfTerminating(HttpResponse response) {
    ...
    // if there is a Transfer-Encoding value, determine whether the final encoding is "chunked", which makes the message
    // self-terminating
    ...
    if (!allTransferEncodingHeaders.isEmpty()) {
      ...
      // per #3 above: "If a message is received with both a Transfer-Encoding header field and a Content-Length header field,
      // the latter MUST be ignored."
      // since the Transfer-Encoding field is present, the message is self-terminating if and only if the final
    ...
    // not checking for multipart/byteranges, since it is seldom used and its use as a message length indicator was removed
    // in RFC 7230
   ...
   * or tabs) surrounding the values will be removed. Empty values (e.g. two consecutive commas, or a value followed by a
   * comma and no other value) will be removed; they will not appear as empty elements in the returned list. If the
   * message contains repeated headers, their values will be added to the returned list in the order in which the headers
   * appear. For example, if a message has headers like:
   * 
   * <pre>
   ...
   * <p>
   * Placing values on multiple header lines is allowed under certain circumstances in RFC 2616 section 4.2, and in RFC
   * 7230 section 3.2.2 quoted here:
   * 
   * <pre>
   ...
   * @return a list of single header values, or an empty list if the header was not present in the message or contained no
   *         values
   */
  ...
  /**
   * Duplicates the status line and headers of an HttpResponse object. Does not duplicate any content associated with that
   * response.
   *
  ...
  /**
   * Determines if the specified header should be removed from the proxied response because it is a hop-by-hop header, as
   * defined by the HTTP 1.1 spec in section 13.5.1. The comparison is case-insensitive, so "Connection" will be treated
   * the same as "connection" or "CONNECTION". From https://www.w3.org/Protocols/rfc2616/rfc2616-sec13.html#sec13.5.1 :
   * 
   * <pre>
  ...
  /**
   * Splits comma-separated header values into tokens. For example, if the value of the Connection header is
   * "Transfer-Encoding, close", this method will return "Transfer-Encoding" and "close". This method strips trims any
   * optional whitespace from the tokens. Unlike {@link #getAllCommaSeparatedHeaderValues(AsciiString, HttpMessage)}, this
   * method only operates on a single header value, rather than all instances of the header in a message.
   *
   ...
   * @param contentLength number of bytes to send in the Content-Length header; should equal the number of bytes in the
   *        ByteBuf
   * @return new http response object
   */
  public static FullHttpResponse createFullHttpResponse(HttpVersion httpVersion, HttpResponseStatus status, String contentType,
      ByteBuf body, int contentLength) {
  ...
  /**
   * Given an HttpHeaders instance, removes 'sdch' from the 'Accept-Encoding' header list (if it exists) and returns the
   * modified instance.
   *
