...
import com.ly.travel.institute.proxy.server.DefaultHostResolver;
import com.ly.travel.institute.proxy.server.DnsSecServerResolver;
import com.ly.travel.institute.proxy.server.HostResolver;
...
public class DefaultHttpProxyServer implements HttpProxyServer {
  ...
  /**
   * Our {@link ServerGroup}. Multiple proxy servers can share the same ServerGroup in order to reuse threads and other
   * such resources.
   */
  ...
  /**
   * JVM shutdown hook to shutdown this proxy server. Declared as a class-level variable to allow removing the shutdown
   * hook when the proxy server is stopped normally.
   */
   ...
   * @param maxHeaderSize
   * @param maxChunkSize
   * @param allowRequestsToOriginServer when true, allow the proxy to handle requests that contain an origin-form URI, as
   *        defined in RFC 7230 5.3.1
  ...
  private DefaultHttpProxyServer(ServerGroup serverGroup, TransportProtocol transportProtocol, InetSocketAddress requestedAddress,
      ...
      int maxInitialLineLength, int maxHeaderSize, int maxChunkSize, boolean allowRequestsToOriginServer, boolean acceptProxyProtocol,
    ...
    if (writeThrottleBytesPerSecond > 0 || readThrottleBytesPerSecond > 0) {
      this.globalTrafficShapingHandler =
          createGlobalTrafficShapingHandler(transportProtocol, readThrottleBytesPerSecond, writeThrottleBytesPerSecond);
    } else {
  ...
  private GlobalTrafficShapingHandler createGlobalTrafficShapingHandler(TransportProtocol transportProtocol,
    ...
    return new GlobalTrafficShapingHandler(proxyToServerEventLoop, writeThrottleBytesPerSecond, readThrottleBytesPerSecond,
        TRAFFIC_SHAPING_CHECK_INTERVAL_MS, Long.MAX_VALUE);
  }
  ...
  public HttpProxyServerBootstrap clone() {
    return new DefaultHttpProxyServerBootstrap(serverGroup, transportProtocol,
        new InetSocketAddress(requestedAddress.getAddress(), requestedAddress.getPort() == 0 ? 0 : requestedAddress.getPort() + 1),
        sslEngineSource, authenticateSslClients, proxyAuthenticator, chainProxyManager, mitmManager, filtersSource, transparent,
        ...
        globalTrafficShapingHandler != null ? globalTrafficShapingHandler.getWriteLimit() : 0, localAddress, proxyAlias,
        maxInitialLineLength, maxHeaderSize, maxChunkSize, allowRequestsToOriginServer);
  }
  ...
  /**
   * Performs cleanup necessary to stop the server. Closes all channels opened by the server and unregisters this server
  ...
  private void doStart() {
    ...
    ChannelInitializer<Channel> initializer = new ChannelInitializer<Channel>() {
      protected void initChannel(Channel ch) {
        new ClientToProxyConnection(DefaultHttpProxyServer.this, sslEngineSource, authenticateSslClients, ch.pipeline(),
            globalTrafficShapingHandler);
      }
    };
    ...
    ChannelFuture future = serverBootstrap.bind(requestedAddress).addListener((ChannelFutureListener) future1 -> {
  ...
  private static class DefaultHttpProxyServerBootstrap implements HttpProxyServerBootstrap {
    ...
    private DefaultHttpProxyServer build() {
      ...
      return new DefaultHttpProxyServer(serverGroup, transportProtocol, determineListenAddress(), sslEngineSource, authenticateSslClients,
          ...
          maxInitialLineLength, maxHeaderSize, maxChunkSize, allowRequestToOriginServer, acceptProxyProtocol, sendProxyProtocol);
    }
