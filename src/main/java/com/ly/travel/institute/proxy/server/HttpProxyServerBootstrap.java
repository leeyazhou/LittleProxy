...
import com.ly.travel.institute.proxy.server.impl.ServerGroup;
...
public interface HttpProxyServerBootstrap {
  ...
  /**
   * Specify a custom {@link ServerGroup} to use for managing this server's resources and such. If one isn't provided, a
   * default one will be created using the {@link ThreadPoolConfiguration} provided
   * 
  ...
  /**
   * Specifies if the proxy server should accept a proxy protocol header. Once set it works with request that include a
   * proxy protocol header. The proxy server reads an incoming proxy protocol header from the client.
   * 
