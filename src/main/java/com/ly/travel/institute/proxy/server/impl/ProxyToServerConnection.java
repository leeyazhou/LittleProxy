...
import com.ly.travel.institute.proxy.server.UnknownTransportProtocolException;
import com.ly.travel.institute.proxy.server.extras.HAProxyMessageEncoder;
...
import static com.ly.travel.institute.proxy.server.impl.ConnectionState.AWAITING_CONNECT_OK;
import static com.ly.travel.institute.proxy.server.impl.ConnectionState.AWAITING_INITIAL;
import static com.ly.travel.institute.proxy.server.impl.ConnectionState.CONNECTING;
import static com.ly.travel.institute.proxy.server.impl.ConnectionState.DISCONNECTED;
import static com.ly.travel.institute.proxy.server.impl.ConnectionState.HANDSHAKING;
...
public class ProxyToServerConnection extends ProxyConnection<HttpResponse> {
  ...
  /**
   * Disables SNI when initializing connection flow in {@link #initializeConnectionFlow()}. This value is set to true when
   * retrying a connection without SNI to work around Java's SNI handling issue (see
   * {@link #connectionFailed(Throwable)}).
   */
  private volatile boolean disableSni = false;

  /**
   * While we're in the process of connecting, it's possible that we'll receive a new message to write. This lock helps us
   * synchronize and wait for the connection to be established before writing the next message.
   */
  ...
  /**
   * While we're doing a chunked transfer, this keeps track of the initial HttpResponse object for our transfer (which is
   * useful for its headers).
   */
  ...
  static ProxyToServerConnection create(DefaultHttpProxyServer proxyServer, ClientToProxyConnection clientConnection,
    ...
    return new ProxyToServerConnection(proxyServer, clientConnection, serverHostAndPort, chainedProxies.poll(), chainedProxies,
  ...
  protected ConnectionState readHTTPInitial(HttpResponse httpResponse) {
    ...
    if (httpResponse.decoderResult().isFailure()) {
      ...
      FullHttpResponse substituteResponse =
          ProxyUtils.createFullHttpResponse(HttpVersion.HTTP_1_1, HttpResponseStatus.BAD_GATEWAY, "Unable to parse response from server");
  ...
  /**
   * <p>
   * Responses to HEAD requests aren't supposed to have content, but Netty doesn't know that any given response is to a
   * HEAD request, so it needs to be told that there's no content so that it doesn't hang waiting for it.
   * </p>
   *
   * <p>
  ...
  /**
   * Keeps track of the current HttpResponse so that we can associate its headers with future related chunks for this same
   * transfer.
   */
  ...
  private void initializeConnectionFlow() {
    ...
    if (ProxyUtils.isCONNECT(initialRequest)) {
      ...
      if (isMitmEnabled) {
        ...
        // with SNI enabled.
        if (disableSni) {
          connectionFlow.then(serverConnection.EncryptChannel(proxyServer.getMitmManager().serverSslEngine()));
        } else {
          connectionFlow.then(serverConnection
              .EncryptChannel(proxyServer.getMitmManager().serverSslEngine(parsedHostAndPort.getHost(), parsedHostAndPort.getPort())));
        }

        connectionFlow.then(clientConnection.RespondCONNECTSuccessful).then(serverConnection.MitmEncryptClientChannel);
      } else {
  ...
  private final ConnectionFlowStep ConnectChannel = new ConnectionFlowStep(this, CONNECTING) {
    ...
    protected Future<?> execute() {
      Bootstrap cb = new Bootstrap().group(proxyServer.getProxyToServerWorkerFor(transportProtocol)).resolver(remoteAddressResolver);
  ...
  private final ConnectionFlowStep HTTPCONNECTWithChainedProxy = new ConnectionFlowStep(this, AWAITING_CONNECT_OK) {
    protected Future<?> execute() {
      ...
      boolean isMitmEnabled = currentFilters.proxyToServerAllowMitm() && mitmManager != null;
      /*
       * We ignore the LastHttpContent which we read from the client connection when we are negotiating connect (see
       * readHttp() in ProxyConnection). This cannot be ignored while we are doing MITM + Chained Proxy because the
       * HttpRequestEncoder of the ProxyToServerConnection will be in an invalid state when the next request is written.
       * Writing the EmptyLastContent resets its state.
       */
      if (isMitmEnabled) {
   ...
   */
  private final ConnectionFlowStep SOCKS4CONNECTWithChainedProxy = new ConnectionFlowStep(this, AWAITING_CONNECT_OK) {
    ...
    protected Future<?> execute() {
      ...
      DefaultSocks4CommandRequest connectRequest =
          new DefaultSocks4CommandRequest(Socks4CommandType.CONNECT, destinationAddress.getHostString(), destinationAddress.getPort());
   ...
   */
  private final ConnectionFlowStep SOCKS5InitialRequest = new ConnectionFlowStep(this, AWAITING_CONNECT_OK) {
   ...
   */
  private final ConnectionFlowStep SOCKS5SendPasswordCredentials = new ConnectionFlowStep(this, AWAITING_CONNECT_OK) {

    @Override
    protected Future<?> execute() {
      DefaultSocks5PasswordAuthRequest authRequest =
          new DefaultSocks5PasswordAuthRequest(username != null ? username : "", password != null ? password : "");
  ...
  /**
   * Establishes a SOCKS5 connection after {@link #SOCKS5InitialRequest} and (optionally)
   * {@link #SOCKS5SendPasswordCredentials} have completed.
   */
  private final ConnectionFlowStep SOCKS5CONNECTRequestWithChainedProxy = new ConnectionFlowStep(this, AWAITING_CONNECT_OK) {

    @Override
    protected Future<?> execute() {
      InetSocketAddress destinationAddress = unresolvedAddressFor(serverHostAndPort);
      DefaultSocks5CommandRequest connectRequest = new DefaultSocks5CommandRequest(Socks5CommandType.CONNECT, Socks5AddressType.DOMAIN,
          destinationAddress.getHostString(), destinationAddress.getPort());
  ...
  /**
   * Convenience method to prepare to retry this connection. Closes the connection's channel and sets up the connection
   * again using {@link #setupConnectionParameters()}.
   *
  ...
  /**
   * Initialize our {@link ChannelPipeline} to connect the upstream server. LittleProxy acts as a client here.
   *
   ...
   * {@link HttpRequestEncoder} before the {@link HttpObjectAggregator} in the {@link ChannelPipeline}.
   */
  private void initChannelPipeline(ChannelPipeline pipeline, HttpRequest httpRequest) {
    ...
    pipeline.addLast(HTTP_DECODER_NAME, new HeadAwareHttpResponseDecoder(proxyServer.getMaxInitialLineLength(),
   ...
   * </p>
   *
   * @param shouldForwardInitialRequest whether or not we should forward the initial HttpRequest to the server after the
  ...
  void connectionSucceeded(boolean shouldForwardInitialRequest) {
    ...
    // completely dropped (HTTPS CONNECTs). if the initialRequest is reference counted (typically because the
   ...
   * @throws UnknownHostException if hostAndPort could not be resolved, or if the input string could not be parsed into a
   *         host and port.
   */
  ...
  /**
   * Similar to {@link #addressFor(String, DefaultHttpProxyServer)} except that it does not resolve the address.
   * 
  ...
  void switchToWebSocketProtocol() {
    final List<String> orderedHandlersToRemove = Arrays.asList(HTTP_REQUEST_WRITTEN_MONITOR_NAME, HTTP_RESPONSE_READ_MONITOR_NAME,
        HTTP_PROXY_ENCODER_NAME, HTTP_ENCODER_NAME, HTTP_DECODER_NAME);
    if (this.channel.pipeline().get(MAIN_HANDLER_NAME) != null) {
      this.channel.pipeline().replace(MAIN_HANDLER_NAME, "pipe-to-client", new ProxyConnectionPipeHandler(clientConnection));
    }
