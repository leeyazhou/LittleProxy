...
/**
 * Basic {@link SslEngineSource} for testing. The {@link SSLContext} uses self-signed certificates that are generated
 * lazily if the given key store file doesn't yet exist.
 */
public class SelfSignedSslEngineSource implements SslEngineSource {
  ...
  private void initializeKeyStore(File keyStoreLocalFile) {
    ...
    nativeCall("keytool", "-genkey", "-alias", alias, "-keysize", "4096", "-validity", "36500", "-keyalg", "RSA", "-dname",
        "CN=littleproxy", "-keypass", password, "-storepass", password, "-keystore", keyStoreLocalAbsoluteFile.getPath());

    nativeCall("keytool", "-exportcert", "-alias", alias, "-keystore", keyStoreLocalAbsoluteFile.getPath(), "-storepass", password, "-file",
        Paths.get(keyStoreLocalAbsoluteFile.getParent(), "littleproxy_cert").toString());
  }
