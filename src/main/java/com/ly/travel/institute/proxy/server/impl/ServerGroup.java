...
public class ServerGroup {
  ...
  /**
   * The default number of threads to accept incoming requests from clients. (Requests are serviced by worker threads, not
   * acceptor threads.)
   */
  ...
  /**
   * A mapping of {@link TransportProtocol}s to their initialized {@link ProxyThreadPoolFactory}. Each transport uses a
   * different thread pool, since the initialization parameters are different.
   */
  private final EnumMap<TransportProtocol, ProxyThreadPoolFactory> protocolThreadPools = new EnumMap<>(TransportProtocol.class);
   ...
   * ServerGroup name in the Thread name. This constructor does not actually initialize any thread pools; instead, thread
  ...
  /**
   * Retrieves the {@link ProxyThreadPoolFactory} for the specified transport protocol. Lazily initializes the thread
   * pools for the transport protocol if they have not yet been initialized. If the protocol has already been initialized,
   * this method returns immediately, without synchronization. If initialization is necessary, the initialization process
   ...
   */
  private ProxyThreadPoolFactory getThreadPoolsForProtocol(TransportProtocol protocol) {
    ...
    if (protocolThreadPools.get(protocol) == null) {
      synchronized (THREAD_POOL_INIT_LOCK) {
        if (protocolThreadPools.get(protocol) == null) {
          ...
          ProxyThreadPoolFactory threadPools = new ProxyThreadPoolFactory(selectorProvider, incomingAcceptorThreads, incomingWorkerThreads,
  ...
  /**
   * Registers the specified proxy server as a consumer of this server group. The server group will not be shut down until
   * the proxy unregisters itself.
   *
  ...
  private void shutdown(boolean graceful) {
    ...
    for (ProxyThreadPoolFactory threadPools : protocolThreadPools.values()) {
  ...
  /**
   * Retrieves the client-to-proxy acceptor thread pool for the specified protocol. Initializes the pool if it has not yet
   * been initialized.
   * <p>
   * This method is thread-safe; no external locking is necessary.
   *
  ...
  /**
   * Retrieves the client-to-proxy acceptor worker pool for the specified protocol. Initializes the pool if it has not yet
   * been initialized.
   * <p>
   * This method is thread-safe; no external locking is necessary.
   *
  ...
  /**
   * Retrieves the proxy-to-server worker thread pool for the specified protocol. Initializes the pool if it has not yet
   * been initialized.
   * <p>
   * This method is thread-safe; no external locking is necessary.
   *
